...
if(typeof mousebox=='undefined') mousebox = {}
mousebox.MouseBox = function(p) {
	if( p === $_ ) return;
	var me = this;
	this.mice = new Hash();
	this.timer = helpers.Stopwatch.Interval(function() {
		me.p_process();
	},mousebox.MouseBox.MS_PER_RECORD);
}
mousebox.MouseBox.__name__ = ["mousebox","MouseBox"];
mousebox.MouseBox.Ready = function(a,dt) {
	return;
}
mousebox.MouseBox.StaticInitialize = function(cb) {
	mousebox.MouseBox.Ready = cb;
	helpers.Stopwatch.Interval(function() {
		var e = mousebox.MouseBox.LatestMouse;
		if(e == null) return;
		mousebox.MouseBox.Times.push(mousebox.MouseBox.MS_ELAPSED + 0.0);
		mousebox.MouseBox.Movements.push({ x : e.pageX / js.Lib.window.innerWidth, y : e.pageY / js.Lib.window.innerHeight});
		mousebox.MouseBox.MS_ELAPSED += mousebox.MouseBox.MS_PER_RECORD;
		if(mousebox.MouseBox.MS_ELAPSED >= mousebox.MouseBox.MS_PER_DISPATCH) {
			mousebox.MouseBox.Ready(mousebox.MouseBox.Movements,mousebox.MouseBox.Times);
			mousebox.MouseBox.Movements = [];
			mousebox.MouseBox.Times = [];
			mousebox.MouseBox.MS_ELAPSED = 0;
		}
	},mousebox.MouseBox.MS_PER_RECORD);
	new js.JQuery("body").mousemove(function(e) {
		mousebox.MouseBox.LatestMouse = e;
	});
}
mousebox.MouseBox.prototype.mice = null;
mousebox.MouseBox.prototype.timer = null;
mousebox.MouseBox.prototype.Mouse = function(mouse_data) {
	if(this.mice.exists(mouse_data.socketid + "")) this.mice.get(mouse_data.socketid + "").Load(mouse_data); else {
		var mouse = new mousebox.Mouse();
		mouse.Load(mouse_data);
		this.mice.set(mouse_data.socketid + "",mouse);
	}
}
mousebox.MouseBox.prototype.Says = function(who,what) {
	if(this.mice.exists(who + "")) this.mice.get(who + "").Tooltip(what);
	return;
}
mousebox.MouseBox.prototype.p_process = function() {
	var $it0 = this.mice.iterator();
	while( $it0.hasNext() ) {
		var mouse = $it0.next();
		mouse.Step(mousebox.MouseBox.MS_PER_RECORD);
	}
}
mousebox.MouseBox.prototype.__class__ = mousebox.MouseBox;
Std = function() { }
Std.__name__ = ["Std"];
Std["is"] = function(v,t) {
...
mousebox.ChatBox = function(cb) {
	if( cb === $_ ) return;
	var me = this;
	this.cb = cb;
	this.p_generatebox();
	this.dom = new js.JQuery("#" + mousebox.ChatBox.Name);
	this.con = new js.JQuery("#" + mousebox.ChatBox.Name + "-container");
	this.text = new js.JQuery("#" + mousebox.ChatBox.Name + "-text");
	...
	new js.JQuery("window").resize(function(e) {
		me.con.css("bottom","1em");
		me.con.css("right","1em");
	});
	new js.JQuery("window").scroll(function(e) {
		me.con.css("bottom","1em");
		me.con.css("right","1em");
	});
}
mousebox.ChatBox.__name__ = ["mousebox","ChatBox"];
mousebox.ChatBox.prototype.dom = null;
mousebox.ChatBox.prototype.text = null;
mousebox.ChatBox.prototype.con = null;
mousebox.ChatBox.prototype.cb = null;
mousebox.ChatBox.prototype.Says = function(who,what) {
	var txt = this.text.val();
	txt += who + " : " + what + "\n";
	this.text.val(txt);
}
mousebox.ChatBox.prototype.p_generatebox = function() {
	mousebox.Mouse.Container.append("\n\t\t\t<div id='" + mousebox.ChatBox.Name + "-container' style='position : absolute; display : block; bottom : 1em; right : 1em;'>\n\t\t\t\t<ul style='list-style : none;'>\n\t\t\t\t\t<li>\n\t\t\t\t\t\t<textarea id='" + mousebox.ChatBox.Name + "-text' \n\t\t\t\t\t\trows='12' cols='40' style='border : 2px solid blue;' readonly='readyonly'>\n\t\t\t\t\t\t</textarea>\n\t\t\t\t\t</li>\n\t\t\t\t\t<li>\n\t\t\t\t\t\t<textarea id='" + mousebox.ChatBox.Name + "' \n\t\t\t\t\t\trows='4' cols='40' style='border : 2px solid blue;'></textarea>\n\t\t\t\t\t</li>\n\t\t\t\t</ul>\n\t\t\t</div>\n\t\t");
}
mousebox.ChatBox.prototype.__class__ = mousebox.ChatBox;
mousebox.Mouse = function(p) {
	if( p === $_ ) return;
	mousebox.Mouse.Container.append(this.p_generatemouse());
	this.dom = new js.JQuery("#mousebox-surfwith-friends-" + mousebox.Mouse.MiceCount);
	this.text = new js.JQuery("#mousebox-surfwith-friends-text-" + mousebox.Mouse.MiceCount);
	mousebox.Mouse.MiceCount += 1;
	this.mouse_data = new List();
	this.animation = new mousebox.DynamicAnimation();
}
mousebox.Mouse.__name__ = ["mousebox","Mouse"];
mousebox.Mouse.prototype.dom = null;
mousebox.Mouse.prototype.text = null;
mousebox.Mouse.prototype.mouse_data = null;
mousebox.Mouse.prototype.animation = null;
mousebox.Mouse.prototype.timer = null;
mousebox.Mouse.prototype.Load = function(md) {
	var me = this;
	if(md.position.length < 1) return;
	if(this.animation.IsNull()) {
		this.animation.Load(md.position,md.time);
		this.animation.SetFinish(function() {
			if(me.mouse_data.isEmpty()) me.animation.Null(); else {
				var md1 = me.mouse_data.pop();
				me.animation.Load(md1.position,md1.time);
			}
		});
	} else {
		this.mouse_data.push(md);
		if(this.mouse_data.length > 50) this.mouse_data.pop();
	}
}
mousebox.Mouse.prototype.Step = function(t) {
	this.animation.Step(t);
	var p = this.animation.Position();
	if(p != null) this.p_position(p);
}
mousebox.Mouse.prototype.Tooltip = function(t) {
	this.text.show();
	this.text.html(t);
	this.text.hide(t.length * 20 < 60000?t.length * 20:60000);
}
mousebox.Mouse.prototype.Hide = function() {
	this.text.hide();
}
mousebox.Mouse.prototype.p_position = function(p) {
	if(p != null) {
		var px = Math.floor(p.x * js.Lib.window.innerWidth);
		var py = Math.floor(p.y * js.Lib.window.innerHeight);
		this.dom.css("left",px + "px");
		this.dom.css("top",py + "px");
		this.text.css("left",px + "px");
		this.text.css("top",py + 40 + "px");
	}
	return { x : this.dom.position().left + 0.0, y : this.dom.position().top + 0.0};
}
mousebox.Mouse.prototype.p_generatemouse = function() {
	var output = "<div class='surffriends mousebox'";
	output += " id='mousebox-surfwith-friends-" + mousebox.Mouse.MiceCount + "' ";
	output += "style='position : absolute; display : block; width : 29px; height : 29px; ";
	output += "background-image : url(\"images/cursor-arrow.png\"); background-size : 29px 29px;'>";
	output += "</div>";
	output += "<div class='surffriends textbox'";
	output += " id='mousebox-surfwith-friends-text-" + mousebox.Mouse.MiceCount + "' ";
	output += "style='position : absolute; display : block; width : 15em; background-color : #121; opacity : 0.85; color : #fff;'>";
	output += "</div>";
	return output;
}
mousebox.Mouse.prototype.__class__ = mousebox.Mouse;
...
mousebox.Animation.__name__ = ["mousebox","Animation"];
mousebox.Animation.prototype.time = null;
mousebox.Animation.prototype.path = null;
mousebox.Animation.prototype.duration = null;
mousebox.Animation.prototype.finished = null;
mousebox.Animation.prototype.isnull = null;
mousebox.Animation.prototype.SetFinish = function(f) {
	this.finished = f;
}
mousebox.Animation.prototype.Null = function() {
	this.time = 0.0;
	this.isnull = true;
	this.path = function(t) {
		return null;
...
mousebox.Animation.prototype.IsNull = function() {
	return this.isnull;
}
mousebox.Animation.prototype.Step = function(t) {
	if(this.time > this.duration) {
		if(this.finished != null) this.finished();
	}
	this.time += t;
}
mousebox.Animation.prototype.Position = function() {
	return this.path(this.time);
}
mousebox.Animation.prototype.__class__ = mousebox.Animation;
if(typeof helpers=='undefined') helpers = {}
helpers.Stopwatch = function() { }
helpers.Stopwatch.__name__ = ["helpers","Stopwatch"];
helpers.Stopwatch.Start = function() {
	helpers.Stopwatch.TIME = haxe.Timer.stamp();
	return helpers.Stopwatch.TIME;
}
helpers.Stopwatch.Stop = function() {
	var startTime = helpers.Stopwatch.TIME;
	var difference = helpers.Stopwatch.Start() - startTime;
	return difference;
}
helpers.Stopwatch.Interval = function(cb,duration) {
	var timer = new haxe.Timer(duration);
	timer.run = cb;
	return timer;
}
helpers.Stopwatch.prototype.__class__ = helpers.Stopwatch;
mousebox.Socket = function(where) {
	if( where === $_ ) return;
	this.socket = io.connect(where);
}
mousebox.Socket.__name__ = ["mousebox","Socket"];
mousebox.Socket.prototype.socket = null;
mousebox.Socket.prototype.on = function(what,call) {
	this.socket.on(what,call);
}
mousebox.Socket.prototype.emit = function(what,data) {
	this.socket.emit(what,data);
}
mousebox.Socket.prototype.__class__ = mousebox.Socket;
mousebox.SurfBox = function() { }
mousebox.SurfBox.__name__ = ["mousebox","SurfBox"];
mousebox.SurfBox.FireInteractionEvent = function(interaction) {
	interaction["socketid"] = mousebox.SurfBox.SocketId;
	mousebox.SurfBox.SOCKET.emit("interaction up",interaction);
}
mousebox.SurfBox.main = function() {
	mousebox.SurfBox.SOCKET.on("query location down",function(id) {
		mousebox.SurfBox.SocketId = id;
		var data = { socketid : mousebox.SurfBox.SocketId, url : js.Lib.window.location.hostname + js.Lib.window.location.pathname};
		mousebox.SurfBox.SOCKET.emit("query location up",data);
	});
	mousebox.SurfBox.SOCKET.on("new user join down",function(data) {
		haxe.Log.trace(data,{ fileName : "SurfBox.hx", lineNumber : 47, className : "mousebox.SurfBox", methodName : "main"});
	});
	mousebox.SurfBox.SOCKET.on("interaction down",function(interaction) {
		var who = interaction.socketid;
		if(who == null) return;
		if(interaction.message != null) {
			mousebox.SurfBox.CHATBOX.Says(who,interaction.message);
			mousebox.SurfBox.MOUSEBOX.Says(who,interaction.message);
		}
		if(interaction.mousedata != null) mousebox.SurfBox.MOUSEBOX.Mouse(interaction.mousedata);
	});
}
mousebox.SurfBox.prototype.__class__ = mousebox.SurfBox;
...
mousebox.DynamicAnimation = function(p) {
	if( p === $_ ) return;
	mousebox.Animation.call(this);
}
mousebox.DynamicAnimation.__name__ = ["mousebox","DynamicAnimation"];
mousebox.DynamicAnimation.__super__ = mousebox.Animation;
for(var k in mousebox.Animation.prototype ) mousebox.DynamicAnimation.prototype[k] = mousebox.Animation.prototype[k];
mousebox.DynamicAnimation.prototype.Load = function(signal,time) {
	var me = this;
	if(signal.length == 0) return;
	if(signal.length == 1) this.path = function(t) {
		return signal[0];
	};
	this.isnull = false;
	this.time = 0.0;
	var start = time[0];
	var finish = time[time.length - 1];
	this.duration = finish - start;
	var index_finder = null;
	index_finder = function(t,ind_start,ind_end) {
		if(index_finder == null) haxe.Log.trace("FFFFFUUUCK!",{ fileName : "DynamicAnimation.hx", lineNumber : 24, className : "mousebox.DynamicAnimation", methodName : "Load"});
		if(t < 0 || t > me.duration) return null;
		if(ind_start + 1 == ind_end) return ind_start;
		var i = Math.floor((ind_start + ind_end) / 2);
		if(t == time[i] - start) return i;
		if(time[i] - start < t && t < time[i + 1] - start) return i;
		if(time[i] - start > t) return index_finder(t,ind_start,i);
		if(time[i] - start < t) return index_finder(t,i,ind_end);
		return null;
	};
	this.path = function(t) {
		if(t > me.duration) return null;
		var index = index_finder(t,0,time.length - 1);
		var p0 = signal[index];
		var pf = signal[index + 1];
		var dd = time[index + 1] - time[index];
		return { x : (pf.x - p0.x) / dd * (t - time[index]) + p0.x, y : (pf.y - p0.y) / dd * (t - time[index]) + p0.y};
	};
}
mousebox.DynamicAnimation.prototype.Position = function() {
	return this.path(this.time);
}
mousebox.DynamicAnimation.prototype.__class__ = mousebox.DynamicAnimation;
$_ = {}
js.Boot.__res = {}
js.Boot.__init();
{
...
js.Lib.onerror = null;
mousebox.MouseBox.LatestMouse = null;
mousebox.MouseBox.MS_PER_RECORD = 45;
mousebox.MouseBox.MS_PER_DISPATCH = 2000;
mousebox.MouseBox.MS_ELAPSED = 0;
mousebox.MouseBox.Movements = new Array();
mousebox.MouseBox.Times = new Array();
mousebox.ChatBox.Name = "SurfBox-Chatbox-Faggot";
mousebox.Mouse.MiceCount = 0;
mousebox.Mouse.Container = new js.JQuery("body");
helpers.Stopwatch.TIME = haxe.Timer.stamp();
mousebox.SurfBox.SOCKET = new mousebox.Socket("http://crunchymall.com:8000");
mousebox.SurfBox.MOUSEBOX = (function() {
	mousebox.MouseBox.StaticInitialize(function(pos,time) {
		var interaction = { socketid : 0, message : null, timestamp : null, mousedata : { socketid : 0, position : pos, time : time}};
		mousebox.SurfBox.FireInteractionEvent(interaction);
	});
	return new mousebox.MouseBox();
})();
mousebox.SurfBox.CHATBOX = new mousebox.ChatBox(function(msg) {
	var interaction = { socketid : null, message : msg, timestamp : null, mousedata : null};
	mousebox.SurfBox.FireInteractionEvent(interaction);
});
mousebox.SurfBox.SocketId = 0;
mousebox.SurfBox.main()
