...
mousebox.SurfBox.main = function() {
	MyJoinedRoomCallback = function(socketid) {
		return;
	};
	MyInteractionCallback = function(interaction) {
		if(interaction.mousedata != null) mousebox.SurfBox.MOUSE.Mouse(interaction.mousedata);
		if(interaction.message != null) mousebox.SurfBox.MOUSE.Says(interaction.socketid,interaction.message);
	};
}
...
helpers.Stopwatch.TIME = haxe.Timer.stamp();
mousebox.SurfBox.SOCKETID = (function() {
	helpers.Stopwatch.Interval(function() {
		mousebox.SurfBox.SOCKETID = suckitid;
	},100);
	return 0;
})();
mousebox.SurfBox.MOUSE = (function() {
	mousebox.MouseBox.StaticInitialize(function(pos,time) {
		if(mousebox.SurfBox.SOCKETID == 0) return;
		var idata = { socketid : mousebox.SurfBox.SOCKETID, message : null, timestamp : null, mousedata : { socketid : mousebox.SurfBox.SOCKETID, position : pos, time : time}};
		MyFireInteractionEvent(idata);
	});
	return new mousebox.MouseBox();
})();
mousebox.SurfBox.CHAT = new mousebox.ChatBox(function(s) {
	if(mousebox.SurfBox.SOCKETID == 0) return;
	var idata = { socketid : mousebox.SurfBox.SOCKETID, message : s, timestamp : null, mousedata : null};
	MyFireInteractionEvent(idata);
});
