...
mousebox.MouseBox.StaticInitialize = function(cb) {
	...
	tools.Stopwatch.Interval(function() {
		mousebox.MouseBox.RecordFlag = true;
	},31);
	new js.JQuery("body").mousemove(function(e) {
		if(mousebox.MouseBox.RecordFlag) {
			mousebox.MouseBox.Movements.push({ x : e.pageX + 0.0, y : e.pageY + 0.0});
			mousebox.MouseBox.RecordFlag = false;
		}
		if(mousebox.MouseBox.Movements.length == 64) {
			mousebox.MouseBox.Ready(mousebox.MouseBox.Movements);
			mousebox.MouseBox.Movements.clear();
		}
	});
}
...
mousebox.Mouse = function(p) {
	...
	this.animation = new mousebox.LinearAnimation(2000);
	this.timer = tools.Stopwatch.Interval(function() {
		if(me.mouse_data.isEmpty()) me.animation.Null(); else {
			var mouse = me.mouse_data.pop();
			me.animation.Load(mouse.position);
		}
	},2000);
}
...
mousebox.LinearAnimation = function(duration) {
	if( duration === $_ ) return;
	mousebox.Animation.call(this);
	this.duration = duration;
}
mousebox.LinearAnimation.__name__ = ["mousebox","LinearAnimation"];
mousebox.LinearAnimation.__super__ = mousebox.Animation;
for(var k in mousebox.Animation.prototype ) mousebox.LinearAnimation.prototype[k] = mousebox.Animation.prototype[k];
mousebox.LinearAnimation.prototype.duration = null;
mousebox.LinearAnimation.prototype.Load = function(pvec) {
	var me = this;
	this.path = function(t) {
		if(t >= me.duration) return null;
		var index = Math.floor(t / me.duration * (pvec.length - 1));
		var p0 = pvec[index];
		var pf = pvec[index + 1];
		return { x : (pf.x - p0.x) / me.duration * t + p0.x, y : (pf.y - p0.y) / me.duration * t + p0.y};
	};
}
mousebox.LinearAnimation.prototype.__class__ = mousebox.LinearAnimation;
